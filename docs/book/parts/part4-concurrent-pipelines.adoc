...
The compression stage is a plain `Stage` with one input and one output port. Its
`run_` is the pop / transform / emplace / `PROCEED` pattern from earlier, falling
back to `SLEEP` when there is nothing to compress:
