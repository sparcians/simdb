 ...
 *                                           For contig, only when the change is not a CONTIG_BOOKENDS shift pattern.
 ...
 * \endverbatim
 *
 * Scalars only ever emit CLOSED, FULL, or CARRY. Contig and sparse share tier-2 encodings
 * (CONTAINER_SWAP, CONTAINER_MULTI_SWAP) with identical wire layout; contig adds tier-3
 * FIFO encodings; sparse adds tier-4 index-based encodings.
 *
 ...
 * - BlobReplayer.hpp:     replayer for testing
 * - blob_iterator.py:     replayer for UI
 */
